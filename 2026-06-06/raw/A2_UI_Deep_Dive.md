...
- 카카오톡 말풍선 안에서 쇼핑, 예약, 결제 승인, 비교표, 배송지 입력 같은 UI를 agent가 동적으로 제안하게 만들 수 있다.
...
- Catalog: 조직별 component schema. 카카오는 Kakao Design System catalog를 만들어야 한다.
...
- CEO 보고에서는 “유력한 방향이지만 pre-v1로 빠르게 변경 중”이라고 표현하는 것이 안전하다.
...
| --- | --- | --- |
| 선물하기 agent | 추천 상품 카드, 옵션 선택, 배송지 확인, 메시지 작성 UI를 agent가 생성 | Kakao commerce component catalog, price/stock validation, Pay approval 분리 |
...
- 카카오톡 mobile/web/native renderer가 동일한 payload를 각 환경의 native component로 렌더링해야 한다.

## 6. Recommended Actions

1. Kakao A2-UI Catalog: Text, Button, ProductCard, OptionSelector, DateTimePicker, AddressSelector, ApprovalSummary 등 기본 catalog를 정의한다.
2. Renderer PoC: 카카오톡 웹/모바일 중 하나에서 A2-UI payload를 native UI로 렌더링하는 PoC를 만든다.
